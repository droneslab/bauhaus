...
use super::{
    messages::{ShutdownMsg, TrackingStateMsg, TrajectoryMsg},
    tracking_backend::TrackingState,
};
use crate::{
    map::{map::Id, pose::Pose, read_only_lock::ReadWriteMap},
    modules::imu::ImuCalib,
};
use core::{
...
impl Actor for ShutdownActor {
    ...
    fn spawn(system: System, map: Self::MapRef) {
        let mut actor = ShutdownActor {
            ...
            camera_trajectory_filename: SETTINGS
                .get::<String>(SYSTEM, "camera_trajectory_file_name"),
            keyframe_trajectory_filename: SETTINGS
                .get::<String>(SYSTEM, "keyframe_trajectory_file_name"),
        };
        loop {
...
impl ShutdownActor {
    fn handle_message(&mut self, message: MessageBox) -> bool {
        if message.is::<TrajectoryMsg>() {
            let msg = message
                .downcast::<TrajectoryMsg>()
                .unwrap_or_else(|_| panic!("Could not downcast shutdown actor message!"));
            ...
            match msg.state {
                TrackingState::Lost => {
                    ...
                    self.trajectory_poses
                        .push(self.trajectory_poses.last().unwrap().clone());
                    self.trajectory_times
                        .push(self.trajectory_times.last().unwrap().clone());
                    self.trajectory_keyframes
                        .push(self.trajectory_keyframes.last().unwrap().clone());
                }
                _ => {}
            }
            return false;
            ...
            let file_camera = File::create(
                Path::new(&self.results_folder).join(&self.camera_trajectory_filename),
            );
            let file_keyframe = File::create(
                Path::new(&self.results_folder).join(&self.keyframe_trajectory_filename),
            );
                ...
                Ok(file) => {
                    ...
                    for i in 0..self.trajectory_poses.len() {
                        ...
                        while !lock.has_keyframe(ref_kf) {
                            let (parent_id, pose_relative_to_parent) =
                                lock.get_deleted_keyframe_info(ref_kf);
                        ...
                        if self.sensor.is_imu() {
                            let twc =
                                (imu_calib.as_ref().unwrap().tbc * self.trajectory_poses[i] * trw)
                                    .inverse();
