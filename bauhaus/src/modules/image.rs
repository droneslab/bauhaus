use opencv::{
    core::{DMatch, KeyPoint, MatExprTraitConst, MatTraitConst, Point2f, RNGTrait, Vector},
    imgcodecs,
    imgproc::resize,
    prelude::Mat,
...
pub fn draw_optical_flow(
    last_image: &Mat,
    curr_image: &Mat,
    last_points: &Vector<Point2f>,
    curr_points: &Vector<Point2f>,
    filename: &str,
) -> Result<(), opencv::Error> {
    let mut curr_image_color = {
        let mut dst = Mat::default();
        opencv::imgproc::cvt_color(curr_image, &mut dst, opencv::imgproc::COLOR_GRAY2BGR, 0)
            .unwrap();
        dst
    };
    let last_image_color = {
        let mut dst = Mat::default();
        opencv::imgproc::cvt_color(last_image, &mut dst, opencv::imgproc::COLOR_GRAY2BGR, 0)
            .unwrap();
        dst
    };
    let mut mask = Mat::zeros(
        last_image_color.rows(),
        last_image_color.cols(),
        last_image_color.typ(),
    )?
    .to_mat()?;
    ...
    for _ in 0..last_points.len() {
        ...
        colors.push(opencv::core::Scalar_::<f64>::new(
            r as f64, g as f64, b as f64, 0.0,
        ));
    }
