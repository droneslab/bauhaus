...
pub fn load_config(
    system_fn: &String,
    camera_fn: &String,
) -> Result<(Vec<ActorConf>, Vec<ModuleConf>, String), Box<dyn std::error::Error>> {
...
fn load_system_settings(system_fn: &String) -> (Vec<ActorConf>, Vec<ModuleConf>, String) {
    ...
    add_setting_bool(
        SYSTEM,
        "localization_only_mode",
        &system_settings["localization_only_mode"],
    );
    add_setting_string(
        SYSTEM,
        "vocabulary_file",
        &system_settings["vocabulary_file"],
    );
    add_setting_string(
        SYSTEM,
        "keyframe_trajectory_file_name",
        &system_settings["keyframe_trajectory_file_name"],
    );
    add_setting_string(
        SYSTEM,
        "camera_trajectory_file_name",
        &system_settings["camera_trajectory_file_name"],
    );
    add_setting_string(SYSTEM, "results_folder", &system_settings["results_folder"]);
    add_setting_string(
        SYSTEM,
        "first_actor_name",
        &system_settings["first_actor_name"],
    );
    add_setting_f64(SYSTEM, "fps", &system_settings["fps"]);
    add_setting_bool(
        SYSTEM,
        "check_deadlocks",
        &system_settings["check_deadlocks"],
    );
...
fn load_camera_settings(camera_fn: &String, module_println: &mut Vec<ModuleConf>) {
    ...
    add_setting_i32(
        "CAMERA",
        "stereo_overlapping_begin",
        &yaml_document["stereo_overlapping_begin"],
    );
    add_setting_i32(
        "CAMERA",
        "stereo_overlapping_end",
        &yaml_document["stereo_overlapping_end"],
    );
    ...
    match SETTINGS.get::<Sensor>(SYSTEM, "sensor") {
        Sensor(FrameSensor::Stereo, _) | Sensor(FrameSensor::Rgbd, _) => {
            add_setting_f64(
                ...
                &yaml_document["stereo_baseline_times_fx"],
            );
    ...
    if !yaml_document["new_height"].is_badvalue() {
        ...
        SETTINGS.insert(
            &"CAMERA",
            &"fy",
            yaml_document["fy"].as_f64().unwrap() * scale_row_factor,
        );
        SETTINGS.insert(
            &"CAMERA",
            &"cy",
            yaml_document["cy"].as_f64().unwrap() * scale_row_factor,
        );
    ...
    if !yaml_document["new_width"].is_badvalue() {
        ...
        SETTINGS.insert(
            &"CAMERA",
            &"fx",
            yaml_document["fx"].as_f64().unwrap() * scale_col_factor as f64,
        );
        SETTINGS.insert(
            &"CAMERA",
            &"cx",
            yaml_document["cx"].as_f64().unwrap() * scale_col_factor as f64,
        );
    ...
    add_setting_imu_matrix(
        "IMU",
        "T_b_c1",
        vec![
            &yaml_document["imu__T_b_c1_row1"],
            &yaml_document["imu__T_b_c1_row2"],
            &yaml_document["imu__T_b_c1_row3"],
    ...
    if SETTINGS.get::<Sensor>(SYSTEM, "sensor").is_imu() {
        add_setting_f64(
            ...
            &yaml_document["initialPositionSigma"],
        );
        add_setting_f64(
            "IMU",
            "initialRollPitchSigma",
            &yaml_document["initialRollPitchSigma"],
        );
        add_setting_f64("IMU", "initialYawSigma", &yaml_document["initialYawSigma"]);
        add_setting_f64(
            "IMU",
            "initialVelocitySigma",
            &yaml_document["initialVelocitySigma"],
        );
        add_setting_f64(
            "IMU",
            "initialAccBiasSigma",
            &yaml_document["initialAccBiasSigma"],
        );
        add_setting_f64(
            "IMU",
            "initialGyroBiasSigma",
            &yaml_document["initialGyroBiasSigma"],
        );
        add_setting_f64(
            "IMU",
            "imu_integration_sigma",
            &yaml_document["imu_integration_sigma"],
        );
        add_setting_f64(
            "IMU",
            "imu_bias_init_sigma",
            &yaml_document["imu_bias_init_sigma"],
        );
    }
...
fn add_settings(settings: &Vec<Yaml>, namespace: &String) -> Option<()> {
    for s in 0..settings.len() {
        ...
        let s_name = setting[&Yaml::String("name".to_string())]
            .as_str()?
            .to_string();
        let s_value = &setting[&Yaml::String("value".to_string())];
        let s_type = setting[&Yaml::String("type".to_string())]
            .as_str()?
            .to_string();
        ...
        match s_type.as_ref() {
            ...
            _ => panic!(
                "Incompatible type {} for setting {} in {}",
                s_type,
                s_value.as_str()?.to_string(),
                s_name
            ),
        };
    }
