...
use opencv::{core::*, hub_prelude::MatExprTraitConst, prelude::*};
...
impl DVMatrix {
    ...
    pub fn new_expr_res<T: opencv::prelude::MatExprTraitConst>(
        mat: opencv::core::MatExprResult<T>,
    ) -> Self {
...
impl DVVectorOfKeyPoint {
    ...
    pub fn get(&self, index: usize) -> Result<KeyPoint, opencv::Error> {
        self.0.get(index)
    }
    pub fn convert(
        &self,
        vec_of_points: &mut opencv::types::VectorOfPoint2f,
        index: &opencv::types::VectorOfi32,
    ) -> Result<(), opencv::Error> {
    ...
    > DVVector3<T>
{
    ...
    pub fn is_zero(&self) -> bool {
        self.0 == nalgebra::Vector3::<T>::zeros()
    }
    ...
    > DVMatrix3<T>
{
    ...
    pub fn is_zero(&self) -> bool {
        self.0 == nalgebra::Matrix3::<T>::zeros()
    }

    pub fn vec(&self) -> &nalgebra::Matrix3<T> {
        &self.0
    }
    pub fn transpose(&self) -> nalgebra::Matrix3<T> {
        self.0.transpose()
    }
...
impl From<&DVMatrix3<f64>> for [[f32; 3]; 3] {
    fn from(mat: &DVMatrix3<f64>) -> [[f32; 3]; 3] {
        ...
        [
            [
                mat[(0, 0)].clone() as f32,
                mat[(1, 0)].clone() as f32,
                mat[(2, 0)].clone() as f32,
            ],
            [
                mat[(0, 1)].clone() as f32,
                mat[(1, 1)].clone() as f32,
                mat[(2, 1)].clone() as f32,
            ],
            [
                mat[(0, 2)].clone() as f32,
                mat[(1, 2)].clone() as f32,
                mat[(2, 2)].clone() as f32,
            ],
        ]
    }
}
    ...
    > DVMatrix4<T>
{
    ...
    pub fn is_zero(&self) -> bool {
        self.0 == nalgebra::Matrix4::<T>::zeros()
    }

    pub fn vec(&self) -> &nalgebra::Matrix4<T> {
        &self.0
    }
    pub fn transpose(&self) -> nalgebra::Matrix4<T> {
        self.0.transpose()
    }
    pub fn determinant(&self) -> T {
        self.0.determinant()
...
impl From<&DVMatrix4<f64>> for Vec<dvos3binding::ffi::DoubleVec> {
    fn from(mat: &DVMatrix4<f64>) -> Vec<dvos3binding::ffi::DoubleVec> {
        ...
        for i in 0..mat.0.nrows() {
            ...
            res.push(dvos3binding::ffi::DoubleVec { vec: row });
        }
        res
    }
}
    ...
    > DVMatrix7x7<T>
{
    ...
    pub fn is_zero(&self) -> bool {
        self.0 == nalgebra::SMatrix::<T, 7, 7>::zeros()
    }

    pub fn vec(&self) -> &nalgebra::SMatrix<T, 7, 7> {
        &self.0
    }
    pub fn transpose(&self) -> nalgebra::SMatrix<T, 7, 7> {
        self.0.transpose()
    }
    pub fn determinant(&self) -> T {
        self.0.determinant()
    ...
    fn from(mat: &DVMatrixDynamic<u8>) -> opencv::core::Mat {
        ...
        for i in 0..mat.0.nrows() {
            for j in 0..mat.0.ncols() {
                ...
                unsafe {
                    *new_mat
                        .at_2d_unchecked_mut::<u8>(i.try_into().unwrap(), j.try_into().unwrap())
                        .unwrap() = mat.0[(r, c)];
                }
            }
        }
