...
class BridgeSparseOptimizer
{
  public:
    ...
    // edges
    void add_both_sim_edges(int vertex_id1, float kp_pt_x_1, float kp_pt_y_1, float inv_sigma1, int vertex_id2,
                            float kp_pt_x_2, float kp_pt_y_2, float inv_sigma2, float huber_delta);
    void add_edge_monocular_unary(bool robust_kernel, int vertex_id, int keypoint_octave, float keypoint_pt_x,
                                  float keypoint_pt_y, float inv_sigma2, array<double, 3> mp_world_position,
                                  int mappoint_id, float huber_delta);
    void add_edge_monocular_binary(bool robust_kernel, int vertex1, int vertex2, float keypoint_pt_x,
                                   float keypoint_pt_y, float inv_sigma2, float huber_delta);
