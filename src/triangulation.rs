...
pub fn triangulate_v2(&mut self, _context: Context, message: Message) -> ActorResult<()> {
    ...
    if let Some(msg) = message.content_as::<TriangulationMsg>() {
        ...
        for i in 0..mask.rows()
        {
            if *mask.at_2d::<u8>(i,0).unwrap()==0
            {
                ...
                let mut curr_pt = core::Point3f::new(*points_3d.at_2d::<f32>(i,0).unwrap(),*points_3d.at_2d::<f32>(i,1).unwrap(),*points_3d.at_2d::<f32>(i,2).unwrap());
        ...
        let relative_proj_mat = curr_pose.get_transformation_matrix();


        let mut tri_trans_points = Mat::default();
        core::transform(&filtered_points, &mut tri_trans_points, &relative_proj_mat).unwrap();
        
        opencv::calib3d::convert_points_from_homogeneous(&tri_trans_points, &mut points_3d);
        unsafe{
                points_3d.create_nd( &[points_3d.rows(), 3], CV_32FC1);
            }
        ...
        // let mut tri_trans_points = Mat::default();
        // core::transform(&points_3d, &mut tri_trans_points, &relative_proj_mat).unwrap();
...
//======================================================================================
        ...
        {   
                ...
                if curr_ptf.x.abs() <  limit_pt  && curr_ptf.z.abs() <limit_pt  //&& curr_ptf.y.abs() <limit_pt 
                {
                    println!("{:?}", curr_ptf);
                    let x = curr_ptf.x as i32+x_offset;
                    let y  = curr_ptf.z as i32 +y_offset;
   ...
   // This is the handler that will be used by the actor.
// This is the handler that will be used by the actor.
pub fn triangulate_1223(&mut self, _context: Context, message: Message) -> ActorResult<()> {
    if let Some(msg) = message.content_as::<TriangulationMsg>() {
        ...
        let (mut recover_r, mut recover_t, mut mask) = (Mat::default(), Mat::default(), Mat::default());
        let mut triangulated_points = opencv::types::VectorOfPoint3f::new();
        let essential_mat = opencv::calib3d::find_essential_mat_2(
          &proj_points2,
          &proj_points1,
          718.8560, // self.focal,
        ...
        let mut proj_points1_h = Mat::default(); //ones(proj_points1.len() as i32, 3, CV_32FC1).unwrap().to_mat().unwrap();
        opencv::calib3d::convert_points_to_homogeneous(&proj_points1, &mut proj_points1_h);
        let mut proj_points2_h = Mat::default(); //Mat::ones(proj_points2.len() as i32, 3, CV_32FC1).unwrap().to_mat().unwrap();
        opencv::calib3d::convert_points_to_homogeneous(&proj_points2, &mut proj_points2_h);        

        let K_inv = K.inv(core::DecompTypes::DECOMP_CHOLESKY as i32).unwrap().to_mat().unwrap();
        


        println!("{:?}", K_inv);
        println!("{:?}, {:?} , {:?}", proj_points1_h.rows(), proj_points1_h.cols(), proj_points1.len());
        for i in 0..proj_points1.len()
        {
            let indx = i as usize;
            let u1 = proj_points1.get(indx).unwrap().clone();
            let u2 = proj_points2.get(indx).unwrap().clone();
            
            let mut u11 = Mat::new_rows_cols_with_default(1, 3 ,CV_32FC1, opencv::core::Scalar::all(0.0)).unwrap();
            unsafe {
            *u11.at_2d_unchecked_mut::<f32>(0,0).unwrap()  = u1.x;
            *u11.at_2d_unchecked_mut::<f32>(0,1).unwrap() = u1.y;
            *u11.at_2d_unchecked_mut::<f32>(0,2).unwrap()  = 1.0;
            }
            let mut u21 = Mat::new_rows_cols_with_default(1, 3 ,CV_32FC1, opencv::core::Scalar::all(0.0)).unwrap();
            unsafe {
                *u21.at_2d_unchecked_mut::<f32>(0,0).unwrap()  = u2.x;
                *u21.at_2d_unchecked_mut::<f32>(0,1).unwrap()  = u2.y;
                *u21.at_2d_unchecked_mut::<f32>(0,2).unwrap()  = 1.0;
                }

            
            let u1_n = K_inv.dot(&u11).unwrap();
            let u2_n = K_inv.dot(&u21).unwrap();
        ...
        {   
            let curr_pt = points3d.get(i).unwrap();
            opencv::imgproc::circle(&mut self.traj_img, core::Point_::new(curr_pt.x as i32 +x_offset, curr_pt.y as i32 +y_offset), 1, core::Scalar::new(0.0, 0.0, 255.0, 0.0), -1, 8, 0)?;
        }
