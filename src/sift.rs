...
pub async fn sift_extract(_: (), _context: Context, message: Message) -> ActorResult<()> {
    if let Some(msg) = message.content_as::<SiftMsg>() {
        ...
        for path in &msg.img_paths {
            ...
            if kp1.len() > 0 && kp2.len() > 0 {
                ...
                let nades1 = cv_mat_f32_to_na_grayscale(&des1);
                let nades2 = cv_mat_f32_to_na_grayscale(&des2);
