...
//         debug!("Loop closing working on kf {} (frame {})", self.current_kf_id, self.map.read().unwrap().keyframes.get(&self.current_kf_id).unwrap().frame_id);
...
//         if self.map.read().unwrap().num_keyframes() <= 10 {
//             return false;
//         }
...
//         let candidate_kfs = self.map.read().unwrap().kf_db_detect_loop_candidates(self.current_kf_id, min_score);
...
//         let mut loop_connected_keyframes = self.map.read().unwrap().keyframes.get(&self.matched_kf).unwrap().get_covisibility_keyframes(i32::MAX); // vpLoopConnectedKFs
...
//         self.current_connected_kfs = self.map.read().unwrap().keyframes.get(&self.current_kf_id).unwrap().get_covisibility_keyframes(i32::MAX);
...
//         let twc = self.map.read().unwrap().keyframes.get(&self.current_kf_id).unwrap().pose.inverse();
