...
use foxglove::{
    foxglove::items::{
        line_primitive, scene_entity_deletion, ArrowPrimitive, Color, FrameTransform,
        LinePrimitive, MapInfo, Point3, Quaternion, RawImage, SceneEntity, SceneEntityDeletion,
        SceneUpdate, SpherePrimitive, Vector3,
    },
    get_file_descriptor_set_bytes, make_pose,
};
use log::warn;
...
use crate::{
    actors::messages::{ShutdownMsg, VisFeatureMatchMsg, VisFeaturesMsg, VisTrajectoryMsg},
    map::{
        map::Id,
...
use super::messages::{
    LoopClosureEssentialGraphMsg, LoopClosureGBAMsg, LoopClosureMapPointFusionMsg,
    VisTrajectoryTrackingMsg,
};

// Default visual stuff //
// Trajectory and frame
static TRAJECTORY_COLOR: Color = Color {
    r: 1.0,
    g: 0.0,
    b: 0.0,
    a: 1.0,
};
static CONNECTED_KF_COLOR: Color = Color {
    r: 0.0,
    g: 1.0,
    b: 0.0,
    a: 1.0,
};
static CURRENT_FRAME_COLOR: Color = Color {
    r: 0.0,
    g: 1.0,
    b: 0.0,
    a: 1.0,
};
static KEYFRAME_COLOR: Color = Color {
    r: 0.0,
    g: 0.0,
    b: 1.0,
    a: 1.0,
...
static MAPPOINT_COLOR: Color = Color {
    r: 1.0,
    g: 1.0,
    b: 1.0,
    a: 1.0,
};
static MAPPOINT_MATCH_COLOR: Color = Color {
    r: 0.0,
    g: 1.0,
    b: 0.0,
    a: 1.0,
};
static MAPPOINT_LOCAL_COLOR: Color = Color {
    r: 0.0,
    g: 0.0,
    b: 1.0,
    a: 1.0,
};
static MAPPOINT_SIZE: Vector3 = Vector3 {
    x: 0.01,
    y: 0.01,
    z: 0.01,
};
static MAPPOINT_LARGE_SIZE: Vector3 = Vector3 {
    x: 0.1,
    y: 0.1,
    z: 0.1,
};

static TRACKING_TRAJECTORY_COLOR: Color = Color {
    r: 0.0,
    g: 0.8,
    b: 0.0,
    a: 1.0,
};
static TRACKING_TRAJECTORY_COLOR2: Color = Color {
    r: 0.8,
    g: 0.0,
    b: 0.0,
    a: 1.0,
};
static TRACKING_TRAJECTORY_COLOR3: Color = Color {
    r: 0.0,
    g: 0.0,
    b: 0.8,
    a: 1.0,
...
impl Actor for DarvisVisualizer {
    ...
    async fn spawn(system: System, map: Self::MapRef) {
        ...
        let _prev_pose = Point3 {
            x: 0.0,
            y: 0.0,
            z: 0.0,
        };

        let image_draw_type = match SETTINGS
            .get::<String>(VISUALIZER, "image_draw_type")
            .as_str()
        {
            ...
            tokio::spawn({
                ...
                async move {
                    server
                        .serve((
                            [127, 0, 0, 1],
                            SETTINGS.get::<i32>(VISUALIZER, "port") as u16,
                        ))
                        .await
                }
            });
            FoxGloveWriter::new_with_server(server, channels)
                .await
                .expect("Could not create foxglove server")
        } else {
            let mcap_file_path = SETTINGS.get::<String>(VISUALIZER, "mcap_file_path");
            FoxGloveWriter::new_with_mcap_file(&mcap_file_path, channels)
                .expect("Could not create foxglove writer")
        };
        ...
        writer
            .write(TRANSFORM_CHANNEL, transform, 0.0, 0)
            .await
...
impl DarvisVisualizer {
    async fn handle_message(
        ...
        if message.is::<VisFeaturesMsg>() {
            ...
            let msg = message
                ...
                .unwrap_or_else(|_| panic!("Could not downcast visualizer message!"));
            self.debug_gba(msg.kf_id, msg.timestamp)
                .await
    ...
    async fn _update_loop_closure_essential_graph(
        ...
        self.clear_scene(msg.timestamp, LOOP_CLOSURE_CHANNEL)
            .await?;
        ...
        self.writer
            .write(LOOP_CLOSURE_CHANNEL, sceneupdate, msg.timestamp, 1)
            .await?;
        Ok(())
    }
    ...
    async fn update_loop_closure_mappoint_fusion(
        ...
        for (mappoint_id, mappoint) in &lock.mappoints {
            let mp_sphere = {
                if msg.mappoint_matches.contains(&mappoint_id) {
                    ...
                    Some(self.create_sphere(
                        &pose,
                        MAPPOINT_MATCH_COLOR.clone(),
                        MAPPOINT_LARGE_SIZE.clone(),
                    ))
        ...
        self.writer
            .write(LOOP_CLOSURE_CHANNEL, sceneupdate, msg.timestamp, 0)
            .await?;
        Ok(())
    }
    ...
    async fn debug_gba(
        ...
        for (mappoint_id, mappoint) in &map.mappoints {
            ...
            let mp_sphere = {
                if map
                    .get_keyframe(kf_id)
                    .get_mp_match_index(mappoint_id)
                    .is_some()
                {
        ...
        let deletions: Vec<SceneEntityDeletion> = deleted_mappoints
            .map(|id| self.create_scene_entity_deletion(timestamp, format!("mp {}", id)))
            .collect();
        ...
        self.writer
            .write(DEBUG_CHANNEL, sceneupdate, timestamp, 0)
            .await?;
    ...
    async fn update_draw_image(&mut self, msg: VisFeaturesMsg) {
        match self.image_draw_type {
            ...
            ImageDrawType::FEATURES => {
                let image_with_features = image::write_features(&msg.image, &msg.keypoints)
                    .expect("Could not draw features to mat");
    ...
    async fn update_draw_image_with_matches(&mut self, msg: VisFeatureMatchMsg) {
        if matches!(self.image_draw_type, ImageDrawType::FEATURESANDMATCHES)
            && self.prev_image.is_some()
        {
            ...
            self.draw_image(&image_with_matches, "bgr8", 3, msg.timestamp)
                .await
                .expect("Could not draw image with matches to mat");
        };
    }

    async fn update_draw_map(
        &mut self,
        msg: VisTrajectoryMsg,
    ) -> Result<(), Box<dyn std::error::Error>> {
        self.draw_trajectory(
            msg.pose,
            msg.timestamp,
            SETTINGS.get::<bool>(VISUALIZER, "draw_graph"),
        )
        .await?;

        self.plot_map_info(&msg.mappoint_matches, msg.timestamp)
            .await?;

        if !SETTINGS.get::<bool>(VISUALIZER, "draw_only_trajectory") {
            self.draw_mappoints(
                &msg.mappoints_in_tracking,
                &msg.mappoint_matches,
                msg.timestamp,
            )
            .await?;
        }
    ...
    async fn draw_frame_trajectory(
        &mut self,
        frame_pose: Pose,
        timestamp: Timestamp,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        entities_traj.push(self.create_frame_scene_entity(
            timestamp,
            "world",
            format!("cam"),
            &inverse_frame_pose,
        ...
        self.writer
            .write(TRACKING_TRAJECTORY_CHANNEL, sceneupdate, timestamp, 0)
            .await
            .expect("Could not write trajectory to foxglove");
    ...
    async fn draw_trajectory(
        &mut self,
        frame_pose: Pose,
        timestamp: Timestamp,
        draw_graph: bool,
    ) -> Result<(), Box<dyn std::error::Error>> {
        self.clear_scene(timestamp, TRAJECTORY_CHANNEL)
            .await
            .expect("Could not clear scene");
        ...
        self.writer
            .write(TRANSFORM_CHANNEL, transform, timestamp, 0)
            .await
            .expect("Could not write transform");
        ...
        for deleted_kf in deleted_keyframes {
            deletions_traj
                .push(self.create_scene_entity_deletion(timestamp, format!("kf {}", deleted_kf)));
            if draw_graph {
                deletions_graph.push(self.create_scene_entity_deletion(
                    timestamp,
                    format!("kf connections {}", deleted_kf),
                ));
            }
        }
        ...
        self.writer
            .write(TRAJECTORY_CHANNEL, sceneupdate, timestamp, 0)
            .await
            .expect("Could not write trajectory to foxglove");
        ...
        if draw_graph {
            ...
            self.writer
                .write(CONNECTED_KFS_CHANNEL, sceneupdate, timestamp, 0)
                .await
                .expect("Could not write trajectory to foxglove");
        }
    ...
    async fn plot_map_info(
        &mut self,
        tracked_mappoints: &Vec<std::option::Option<(i32, bool)>>,
        timestamp: Timestamp,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.writer
            .write(MAP_INFO_CHANNEL, map_info, timestamp, 0)
            .await?;
        Ok(())
    }

    async fn draw_mappoints(
        &mut self,
        local_mappoints: &BTreeSet<Id>,
        ...
        let mappoint_match_ids = mappoint_matches
            .iter()
        ...
        for (mappoint_id, mappoint) in &map.mappoints {
            ...
            let mp_sphere = {
                if mappoint_match_ids.contains(&mappoint_id) {
                    ...
                    && SETTINGS.get::<bool>(VISUALIZER, "draw_local_mappoints")
                {
                ...
                } else if SETTINGS.get::<bool>(VISUALIZER, "draw_all_mappoints")
                    || !self.previous_mappoints.contains(&mappoint_id)
                {
                    // All other mappoints.
        ...
        self.writer
            .write(MAPPOINTS_CHANNEL, sceneupdate, timestamp, 0)
            .await?;
    ...
    async fn draw_points(
        ...
        for point in points {
            let pose =
                Pose::new_with_default_rot(DVTranslation::new_with(point.x, point.y, point.z));
    ...
    async fn _draw_connected_kfs(
        ...
        let curr_keyframes = map
            ...
            .collect::<HashSet<Id>>();
        let deleted_keyframes = self.previous_keyframes.difference(&curr_keyframes);
        let deletions = deleted_keyframes
            .map(|id| self.create_scene_entity_deletion(timestamp, format!("kf {}", id)))
            .collect();
        self.previous_keyframes = curr_keyframes;
    ...
    async fn draw_image(
        ...
        self.writer
            .write(IMAGE_CHANNEL, image_msg, timestamp, 0)
            .await?;
    ...
    fn create_scene_entity_deletion(
        &self,
        timestamp: Timestamp,
        entity_id: String,
    ) -> SceneEntityDeletion {
    ...
    async fn clear_scene(
        ...
        self.writer
            .write(channel_name, sceneupdate, timestamp, 0)
            .await?;
        Ok(())
    }

    fn create_frame_scene_entity(
        &self,
        timestamp: Timestamp,
        frame_id: &str,
        entity_id: String,
...
impl FoxGloveWriter {
    pub fn new_with_mcap_file(
        path: &str,
        channels_to_add: HashMap<&str, &str>,
    ) -> Result<FoxGloveWriter, Box<dyn std::error::Error>> {
    ...
    pub async fn write<T: prost::Message>(
        ...
        match self {
            FoxGloveWriter::McapFile { writer, channels } => {
                ...
                Ok(writer.write_to_known_channel(
                    &MessageHeader {
            ...
            FoxGloveWriter::Stream {
                ...
                Ok(channel
                    .send(timestamp as u64, &data.encode_to_vec())
                    .await?)
            }
        }
    }
