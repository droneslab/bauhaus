...
//                         // let mut write_lock = actor.map.lock();
...
//                         // for id in ids {
//                         //     actor.map.lock().discard_mappoint(&id);
//                         // }
//                     },
...
//                         // if let Some(mp) = actor.map.lock().mappoints.get_mut(&id) {
...
//                         let num_keypoints = actor.map.lock().keyframes.get(&kf_id).expect(&format!("Could not get kf {}", kf_id)).features.num_keypoints;
//                         let mut write = actor.map.lock();
...
//                         actor.map.lock().replace_mappoint(mp_to_replace, mp);
//                     },
//                     MapWriteTarget::MapPoint__Pose{mp_id, pose} => {
//                         let _timer = timer!("MapActor::Total-UpdateMapPointPose");
//                         actor.map.lock().mappoints.get_mut(&mp_id).unwrap().position = pose.get_translation();
//                     }
//                     _ => {
