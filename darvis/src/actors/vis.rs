    ...
    pub fn visualize(&mut self, _context: Context, message: Message) -> ActorResult<()> {
        if let Some(msg) = message.content_as::<VisMsg>() {
            ...
            if self.traj_pos.is_zero() && self.traj_rot.is_zero() {
                self.traj_pos = msg.new_pose.translation();
                self.traj_rot = msg.new_pose.rotation();
            }
            else {
                self.traj_pos = DVVector3::new((*self.traj_pos) + (*self.traj_rot) * (*msg.new_pose.translation()));
                self.traj_rot = DVMatrix3::new((*msg.new_pose.rotation()) * (*self.traj_rot));
            }
