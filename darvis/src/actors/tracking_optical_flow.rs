...
use crate::{
    ...
    modules::image,
    modules::{imu::ImuModule, relocalization::Relocalization, map_initialization::Initialization, orbmatcher, optimizer, camera::CAMERA_MODULE},
    ActorChannels,
    map::{map::Id, misc::Timestamp, pose::Pose, frame::Frame, features::Features}, MapLock,
};

use super::{tracking_backend::TrackedMapPointData, local_mapping::LOCAL_MAPPING_IDLE, messages::{NewKeyFrameMsg, VisTrajectoryMsg, TrajectoryMsg, InitKeyFrameMsg, ImagePathMsg}};
...
impl Actor for TrackingOpticalFlow {
    ...
    fn new_actorstate(actor_channels: ActorChannels, map: Self::MapRef) -> TrackingOpticalFlow {
    ...
    fn spawn(actor_channels: ActorChannels, map: Self::MapRef) {
        let mut actor = TrackingOpticalFlow::new_actorstate(actor_channels, map);
        ...
        // let mut last_pts: Option<VectorOfPoint2f> = None;
        ...
        'outer: loop {
            ...
            if message.is::<ImagePathMsg>() || message.is::<ImageMsg>(){
                ...
                let (mut current_frame) = if last_img.is_none(){
                    ...
                    // let points_curr = keypoints.into_iter().map(|kp| kp.pt).collect();
                    // (None, points_curr, VectorOfu8::new(), keypoints)
                ...
                else 
                {
                    ...
                    // info!("Actor ref kf id is {:?}", actor.ref_kf_id);
                    ...
                    // info!("last_img is {:?}", last_img.is_some());
                    // info!("points1 is {:?}", points1.len());
                    // info!("image width is {:?}", image.cols());
                    ...
                    if prev_kps.len() < actor.min_num_features as usize 
                    {
                        ...
                        let (mut keypoints_prev, mut descriptors_prev) = actor.extract_features_and_add_to_existing(last_kf_img.clone(), curr_frame_id-1, &prev_kps_copy, &prev_descriptors_copy.as_ref().unwrap());
...
impl TrackingOpticalFlow {
    ...
    fn extract_features_and_add_to_existing(&mut self, image: opencv::core::Mat, curr_frame_id: i32, keypoints_old: &VectorOfKeyPoint, descriptors_old: &Mat) -> (VectorOfKeyPoint, Mat) {
        ...
        let mut new_descriptors = Mat::default();
    ...
    fn track(&mut self, current_frame: &mut Frame, last_frame: &mut Option<Frame>) -> Result<(), Box<dyn std::error::Error>>  {
        ...
        if enough_matches || matches!(self.state, TrackingState::RecentlyLost) {
            ...
            info!("Need new keyframe: {}", need_new_kf);
    ...
    fn update_trajectory_in_logs(
        ...
        if self.actor_channels.actors.get(VISUALIZER).is_some() {
            ...
            self.actor_channels.send(VISUALIZER, Box::new(VisTrajectoryMsg{
                pose: relative_pose , //current_frame.pose.unwrap(),
    ...
    fn track_reference_keyframe(&mut self, current_frame: &mut Frame, last_frame: &mut Frame) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        let mut nmatches=0;
        {
            warn!("Current search_by_bow_f ref kf id is {:?}", self.ref_kf_id);
            let map_read_lock = self.map.read();

            // while !map_read_lock.keyframes.contains_key(&self.ref_kf_id.unwrap()) {
            //     warn!("Reference kf id {:?} not in map", self.ref_kf_id.unwrap());
            //     continue;
            // }
        ...
        // let nmatches = current_frame.mappoint_matches.matches.len();
    ...
    fn update_local_keyframes(&mut self, current_frame: &mut Frame, last_frame: &mut Frame) -> Option<()> {
        ...
        for (kf_id, count) in kf_counter {
            ...
            // if max_kf_id  < kf_id{
                max = count;
                max_kf_id = kf_id;
                // info!("max_kf_id: {}, max:  {}", max_kf_id, max);
            }
        ...
        if max_kf_id != 0 {
            ...
            info!("Current frame ref kf id: {}", current_frame.ref_kf_id.unwrap());
        }
        else
        {
            warn!("No ref kf id found for current frame");
        }
        None
    }
    ...
    //     let (keypoints, descriptors) = self.extract_features(self.current_image.clone().unwrap(), current_frame.frame_id);
    //     // update current frame with new features

    //     let im_width = self.current_image.as_ref().unwrap().cols() as u32;
    //     let im_height = self.current_image.as_ref().unwrap().rows() as u32;

    //    let features = Features::new(DVVectorOfKeyPoint::new(keypoints), DVMatrix::new(descriptors), im_width, im_height, self.sensor);

    //     current_frame.features = features.unwrap();
        ...
        self.last_kf_image = self.current_image.clone();
