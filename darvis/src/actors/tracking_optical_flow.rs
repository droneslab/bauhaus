...
//             let (keypoints, descriptors) = self.extract_features(& curr_img, self.curr_frame_id);
...
//         let (keypoints, descriptors) = self.extract_features(& self.current_frame.image.as_ref().unwrap(), self.curr_frame_id);
