...
impl TrackingGTSAM {
    fn handle_message(&mut self, message: MessageBox) -> bool {
        if message.is::<ImagePathMsg>() || message.is::<ImageMsg>() {
            ...
            let (mut curr_frame, created_kf) = if matches!(self.state, TrackingState::NotInitialized) {
                // If map is not initialized yet, just extract features and try to initialize
                (self.initialize_map(&image, timestamp).unwrap(), false)
            } else {
    ...
    fn initialize_map(&mut self, curr_img: &opencv::core::Mat, timestamp: Timestamp) -> Result<Frame, Box<dyn std::error::Error>> {
        ...
        let init_success = self.initialization.as_mut().unwrap().try_initialize(&curr_frame, &mut self.imu.imu_preintegrated_from_last_kf)?;
        if init_success {
                ...
                (ini_kf_id, curr_kf_id) = match self.initialization.as_mut().unwrap().create_initial_map_monocular(&mut self.map,  &mut self.imu.imu_preintegrated_from_last_kf)? {
                    Some((curr_kf_pose, curr_kf_id, ini_kf_id, local_mappoints, curr_kf_timestamp, map_scale)) => {
...

                        {
                            ...
                            curr_frame.pose = Some(curr_kf_pose);
                            curr_frame.ref_kf_id = Some(curr_kf_id);
    ...
    fn track(
        ...
        let mut curr_frame = Frame::new_no_features(
            self.curr_frame_id, 
            Some(curr_img.clone()),
            timestamp,
        ...
        debug!("Optical flow, tracked {} from original {}", curr_frame.features.num_keypoints, status.len());

        self.imu.preintegrate(&mut imu_measurements, &mut curr_frame, self.last_frame.as_mut().unwrap(), self.map.read()?.last_kf_id);

        let g = 9.81;

        // Solve VIO graph

        self.add_imu_measurements(&curr_frame); // measured_poses, measured_acc, measured_omega, measured_vel, delta_t, 10
        ...
        //     // }
        // }
        Ok((curr_frame, created_keyframe))
    }
    ...
    fn add_imu_measurements(&mut self, curr_frame: &Frame) {
        ...
        self.graph.add_imu_prior(
            bias_key,
            &constantbias,
            &DiagonalNoiseModel::from_sigmas(nalgebra::Vector6::new(0.5, 0.5, 0.5, 0.5, 0.5, 0.5))
        );
        self.initials.insert_imu_bias(bias_key, &constantbias);
        //Reference:
