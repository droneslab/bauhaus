...
use dvcore::{lockwrap::ReadOnlyWrapper, plugin_functions::Function, global_params::*};
use crate::{
    actors::messages::{FeatureMsg, KeyFrameMsg},
    registered_modules::{LOCAL_MAPPING, TRACKING_BACKEND},
    dvmap::{
        map::Map, map_actor::MapWriteMsg, map_actor::{MAP_ACTOR, MapInitializedMsg}, map::Id,
...
use super::messages::{InitialMapMsg};

#[derive(Debug, Clone, Default)]
struct TrackingDefaults {
    ...
    far_points_threshold: f64 // mThFarPoints
}
impl TrackingDefaults {
    pub fn new() -> Self {
        Self {
            ...
            far_points_threshold: GLOBAL_PARAMS.get::<f64>(SYSTEM_SETTINGS, "far_points_threshold"),
...
pub struct DarvisTrackingBack<S: SensorType> {
    ...
    // Feature matching
    temporal_points: Vec<MapPoint<FullMapPoint<S>>>,
    ...
    camera: Camera,
    map_updated : bool,  // TODO: I'm not sure we want to use this
    ...
    global_defaults: TrackingDefaults,
}
...
impl<S: SensorType + 'static> DarvisTrackingBack<S> {
    ...
    fn hidden_new<S2: SensorType> (map: ReadOnlyWrapper<Map<S>>) -> DarvisTrackingBack<S> {
        ...
        let mut bla = DarvisTrackingBack {
            map,
            camera,
            ..Default::default()
        };
        ...
        bla.imu = match S::sensor_type() {
            Sensor::Mono | Sensor::Rgbd | Sensor::Stereo => None,
            _ => Some(ImuModule::<S>::default()),
            // Sensor::ImuRgbd => Some(ImuModule::<ImuRgbdSensor>::default()),
            // Sensor::ImuMono => Some(ImuModule::<ImuMonoSensor>::default()),
        };
    ...
    fn handle_message(&mut self, context: axiom::prelude::Context, msg: Arc<FeatureMsg>) {
        ...
        // match S::sensor_type() {
        //     Sensor::Mono | Sensor::Rgbd | Sensor::Stereo => None,
        //     _ => self.imu.unwrap().preintegrate()
        // };
            // Sensor::ImuRgbd => Some(ImuModule::<ImuRgbdSensor>::default()),
            // Sensor::ImuMono => Some(ImuModule::<ImuMonoSensor>::default()),
        // };
        ...
        let initial_success = match self.state {
            ...
            TrackingState::NotInitialized => {
                match self.initialization.try_initialize(&self.current_frame, &self.camera) {
                    Ok(()) => {
                        // TODO Stereo: should there be a create_initial_map_stereo ?
                        let msg = MapWriteMsg::create_initial_map_monocular(self.initialization.clone(), tracking_actor);
                        map_actor.send_new(msg).unwrap();
                        info!("tracking_backend::handle_message;Sent initialization info to map");
            ...
            TrackingState::Ok => {
                ...
                //     false => self.track_with_motion_model() || self.track_reference_keyframe()
                // };
            ...
            TrackingState::RecentlyLost => {
                ...
                let ok = false;
                // SOFIYA TODO Sensor restructuring
                // let (ok, move_to_lost) = match self.imu.unwrap().ready(&self.map) {
                //     true => (
                //         self.imu.unwrap().predict_state(),
            ...
            TrackingState::Lost => {
                ...
                if self.kfs_in_map() < 10 {
                    ...
                    map_actor.send_new(MapWriteMsg::<S>::reset_active_map()).unwrap();
                } else {
                    self.create_new_map();
                }
        ...
        //     if enough_frames_to_reset_imu && self.current_frame.as_ref().unwrap().id > self.frames_to_reset_imu && S::is_imu() && self.self.map.read().imu_initialized() {
        //         // Load preintegration
        ...
        if success || matches!(self.state, TrackingState::RecentlyLost) {
            ...
            if !last_frame.is_none() && !last_frame.unwrap().pose.is_none() && !self.current_frame.pose.is_none() {
                ...
                // TODO (IMU)
                self.imu.as_mut().unwrap().velocity = Some(self.current_frame.pose.as_ref().unwrap().clone() * last_twc);
            } else {
                self.imu.as_mut().unwrap().velocity = None;
            }
        ...
        if matches!(self.state, TrackingState::Lost) {
            ...
            if S::is_imu() && !self.map.read().imu_initialized {
    ...
    fn track_reference_keyframe(&mut self) -> bool {
        ...
        if self.ref_kf_id.is_some() {
            ...
            if ref_kf.is_some() {
                // TODO: BINDINGS
                // nmatches = orbmatcher::search_by_bow_f(ref_kf.unwrap(), &self.current_frame, &mut vp_mappoint_matches); //0.7, true
            } else {
            ...
            if self.reference_frame.is_some() {
                ...
                // TODO BINDINGS
                // nmatches = orbmatcher::search_for_initialization(0.7, true, self.reference_frame.as_ref().unwrap(), &self.current_frame, &mut self.prev_matched, &mut vp_mappoint_matches, 100);
            } else {
                todo!("fix invalid ref KF assignment");
            }
    ...
    fn track_with_motion_model(&mut self) -> bool {
        ...
        let enough_frames_to_reset_imu = self.current_frame.id <= self.relocalization.last_reloc_frame_id + (self.global_defaults.frames_to_reset_imu as i32);
        if self.map.read().imu_initialized && enough_frames_to_reset_imu {
            ...
            self.current_frame.pose = Some(self.imu.as_ref().unwrap().velocity.unwrap() * self.last_frame.as_ref().unwrap().pose.unwrap());
        }
    ...
    fn search_local_points(&mut self) {
        ...
        let mut track_in_view = HashSet::<Id>::new();
        ...
        for mp_id in &self.local_mappoints {
            ...
            let map_read_lock = self.map.read();
            match self.current_frame.is_in_frustum(*mp_id, 0.5, &*map_read_lock) {
                true => {
                    mps_to_increase_visible.push(&mp_id);
                    to_match += 1;
                    track_in_view.insert(*mp_id);
        ...
        if to_match > 0 {
            let mut th = match S::sensor_type() {
                Sensor::Rgbd | Sensor::ImuRgbd  => 3,
                _ => 1
            };
            // TODO (IMU)
            ...
            } else if !self.map.read().imu_initialized && S::is_imu() {
                th = 10;
            }
            ...
            // TODO: send mps_to_increase_visible to map

            // TODO 10/17: BINDINGS
            // far_points should be false if far_points_threshold > 0
            // let matches = orbmatcher::search_by_projection( // todo: orbmatcher settings are 0.8 and true
            //     &self.current_frame,
    ...
    fn track_local_map(&mut self, map_actor: &Aid) -> bool {
        ...
        let mut inliers = 0;
        if !self.map.read().imu_initialized || (self.current_frame.id <= self.relocalization.last_reloc_frame_id + (self.global_defaults.frames_to_reset_imu as i32)) {
            self.optimizer.optimize_pose(&mut self.current_frame, &self.map)
                .map(|(_,pose)| self.current_frame.pose = Some(pose) );
        ...
        let map_msg = MapWriteMsg::<S>::increase_found(increase_found);
        map_actor.send_new(map_msg).unwrap();

        // Decide if the tracking was succesful
        // More restrictive if there was a relocalization recently
        if self.current_frame.id < self.relocalization.last_reloc_frame_id + (self.global_defaults.max_frames as i32) && self.matches_in_frame<50 {
            return false;
        }
    ...
    fn create_new_keyframe(&mut self, context: axiom::prelude::Context) {
        ...
        // TODO: Need to set this after the keyframe is inserted into the map from LM
        ...
        match actor_msg.as_ref() {
            "KeyFrameMsg" => {
                localmapping.send_new(KeyFrameMsg{ kf: new_kf }).unwrap();
            },
            _ => {
    ...
    fn need_new_keyframe(&self) -> bool {
        ...
        let not_enough_frames_since_last_reloc = (self.current_frame.id < self.relocalization.last_reloc_frame_id + (self.global_defaults.max_frames as i32)) && (kfs_in_map > (self.global_defaults.max_frames as u32));
        let imu_not_initialized = S::is_imu() && !self.map.read().imu_initialized;
    ...
    fn store_pose_info_for_trajectory(&mut self) {
        match self.current_frame.pose {
            Some(pose) => {
                ...
                let ref_kf = lock.get_keyframe(&ref_kf_id).unwrap();
