...
impl Pose {
    ...
    pub fn new_with_quaternion_convert(
        ...
        let rot = nalgebra::geometry::UnitQuaternion::<f64>::from_quaternion(
            nalgebra::Quaternion::<f64>::new(rotation[0], rotation[1], rotation[2], rotation[3]),
        );
    ...
    pub fn new_with_quaternion(
        translation: nalgebra::Vector3<f64>,
        rotation: nalgebra::geometry::UnitQuaternion<f64>,
    ) -> Pose {
        let trans = nalgebra::Translation3::from(translation);
        Pose(nalgebra::IsometryMatrix3::from_parts(
            trans,
            rotation.into(),
        ))
    }
    ...
    pub fn get_translation(&self) -> DVTranslation {
        DVTranslation::new(self.0.translation.vector)
    }
    pub fn get_rotation(&self) -> DVRotation {
        DVRotation::new(*self.0.rotation.matrix())
    }
    pub fn get_quaternion(&self) -> nalgebra::geometry::UnitQuaternion<f64> {
        nalgebra::geometry::UnitQuaternion::from_rotation_matrix(&self.0.rotation)
    }
    ...
    fn group_inverse(&self) -> Pose {
        ...
        Pose::new(invr * self.0.translation.vector * -1.0, invr.into())
    }
...
impl From<dvos3binding::ffi::Pose> for Pose {
    fn from(pose: dvos3binding::ffi::Pose) -> Self {
        ...
        let matrix3 = nalgebra::Matrix3::<f64>::new(
            pose.rotation[0][0].into(),
            pose.rotation[0][1].into(),
            pose.rotation[0][2].into(),
            pose.rotation[1][0].into(),
            pose.rotation[1][1].into(),
            pose.rotation[1][2].into(),
            pose.rotation[2][0].into(),
            pose.rotation[2][1].into(),
            pose.rotation[2][2].into(),
        );
...
impl Sim3 {
    ...
    pub fn new_scaled_rotation(&self) -> Self {
        ...
        let new_pose = Pose::new(*self.pose.get_translation(), new_rot);
...
impl std::fmt::Debug for Sim3 {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        write!(
            ...
            trans[0], trans[1], trans[2], rot.w, rot.i, rot.j, rot.k, self.scale
        )
    }
}
...
pub fn group_exp(omega: &nalgebra::Vector3<f64>) -> nalgebra::UnitQuaternion<f64> {
    ...
    let result = nalgebra::UnitQuaternion::from_quaternion(Quaternion::new(
        real_factor,
        imag_factor * omega[0],
        imag_factor * omega[1],
        imag_factor * omega[2],
