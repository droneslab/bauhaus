...
impl Optimizer {
    ...
    pub fn pose_inertial_optimization_last_frame(&self, frame: &mut Frame, map: &ReadOnlyWrapper<Map>) -> i32 {
        ...
        let vertex = optimizer.add_frame_vertex(1, (*frame.pose.as_ref().unwrap()).into(), false);
                ...
                Some((mp_id, _)) => {
                    ...
                    let edge = match self.sensor.frame() {
                        ...
                        _ => {
                            ...
                            let map_read_lock = map.read();
                            let position = &map_read_lock.get_mappoint(mp_id).unwrap().position;
                            let edge = optimizer.add_edge_monocular_unary(
                                false, 0, keypoint.octave, keypoint.pt.x, keypoint.pt.y,
                                self.inv_level_sigma2[keypoint.octave as usize],
                                (position).into()
                            );
    ...
    pub fn global_bundle_adjustment(&self, map: &Map, loop_kf: i32, iterations: i32) -> BAResult {
        ...
        let mut keyframe_vertices = HashMap::<Id, SharedPtr<VertexSE3Expmap>>::new();
        for (id, kf) in keyframes {
            keyframe_vertices.insert(*id,
                optimizer.add_frame_vertex(
                    *id,
                    (kf.pose).into(),
        ...
        let mut mappoint_vertices = HashMap::<Id, SharedPtr<VertexSBAPointXYZ>>::new();
        // Set MapPoint vertices
        let mappoints = map.get_all_mappoints();
        for (mp_id, mappoint) in mappoints {
            let vertex = optimizer.add_mappoint_vertex(
                mp_id + max_kf_id + 1,
            ...
            if n_edges == 0 {
                optimizer.remove_vertex(vertex);
            } else {
                mappoint_vertices.insert(*mp_id, vertex);
            }
        }
        ...
        for (kf_id, _) in keyframe_vertices {
        ...
        for (mp_id, _) in mappoint_vertices {
            let pose = optimizer.recover_optimized_mappoint_pose(mp_id);
