...
use log::warn;
use opencv::{
    core::{no_array, norm, MatExprResult, Range, Scalar, CV_64F, NORM_L2},
    hub_prelude::{KeyPointTraitConst, MatExprTraitConst, MatTraitConst},
};
use rand::Rng;

use crate::{
    map::{map::Id, pose::Sim3},
    modules::optimizer::LEVEL_SIGMA2,
    registered_actors::CAMERA_MODULE,
};
use opencv::prelude::*;
...
impl Sim3Solver {
    pub fn new(
        ...
        for (index, mp2_id) in matches {
            let mp1_id = match keyframe_mp1[*index as usize] {
                ...
                None => {
                    warn!(
                        "KF {} does not have MP match at index {}. This should not happen?",
                        kf1_id, index
                    );
                    continue;
                }
            ...
            let (kf1_left_idx, _) = map_lock
                .mappoints
                .get(&mp1_id)
                .unwrap()
                .get_index_in_keyframe(kf1_id);
            let (kf2_left_idx, _) = map_lock
                .mappoints
                .get(&mp2_id)
                .unwrap()
                .get_index_in_keyframe(kf2_id);
    ...
    pub fn iterate(
        &mut self,
        num_iterations: i32,
    ) -> Result<(bool, Option<(Vec<bool>, i32)>), opencv::Error> {
        ...
        while self.current_ransac_state.num_iterations < self.ransac_max_its
            && current_iterations < num_iterations
        {
            ...
            for i in 0..3 {
                ...
                self.x_3d_c1[idx].copy_to(&mut p_3d_c1_i.col(i)?)?;

                self.x_3d_c2[idx].copy_to(&mut p_3d_c2_i.col(i)?)?;
            ...
            if self.current_estimation.inliers_count > self.current_ransac_state.best_inliers_count
            {
                ...
                self.current_ransac_state.best_inliers_count =
                    self.current_estimation.inliers_count;
    ...
    fn compute_sim3(&mut self, p1: &Mat, p2: &Mat) -> Result<(), opencv::Error> {
        ...
        self.current_estimation.r_12_i =
        ...
        self.current_estimation.T12_i =
            Mat::new_rows_cols_with_default(4, 4, CV_64F, Scalar::all(0.0))?;
        ...
        self.current_estimation
            .t_12_i
            .copy_to(&mut self.current_estimation.T12_i.col(0)?)?;

        // Step 8.2 T21
        let s_r_inv = res_to_mat(
            (1.0 / self.current_estimation.s_12_i) * self.current_estimation.r_12_i.t()?,
        )?;
