...
use crate::{
    map::read_only_lock::ReadWriteMap,
    modules::{
        bow::Vocabulary,
        camera::{Camera, CameraType},
        imu::IMU,
        module_definitions::{
            FeatureExtractionModule, FullMapOptimizationModule, LocalMapOptimizationModule,
            LoopDetectionModule,
        },
        orbslam_matcher::ORBMatcherTrait,
    },
};
...
pub fn spawn_actor(actor_tag: String, system: System, map: Option<ReadWriteMap>) {
    ...
    match actor_tag.as_ref() {
        ...
        str if str == "orbslam tracking backend".to_string() => {
            crate::actors::tracking_backend::TrackingBackend::spawn(
                system,
                map.expect("Tracking backend needs the map!"),
            )
        }
        ...
        str if str == "orbslam local mapping".to_string() => {
            crate::actors::local_mapping::LocalMapping::spawn(
                system,
                map.expect("Local mapping needs the map!"),
            )
        }
        str if str == "orbslam3 loop closing".to_string() => {
            crate::actors::loop_closing::LoopClosing::spawn(
                system,
                map.expect("Loop closing needs the map!"),
            )
        }
        ...
        str if str == "gtsam tracking frontend".to_string() => {
            crate::actors::tracking_frontend_gtsam::TrackingFrontendGTSAM::spawn(
                system,
        ...
        str if str == SHUTDOWN_ACTOR.to_string() => crate::actors::shutdown::ShutdownActor::spawn(
            system,
            map.expect("Shutdown needs the map!"),
        ),
        str if str == "garbage tracking".to_string() => {
            crate::actors::garbage_tracking::GarbageTracking::spawn(
                system,
                map.expect("Garbage tracking needs the map!"),
            )
        }
        _ => {
...
pub fn new_feature_extraction_module(is_ini: bool) -> Box<dyn FeatureExtractionModule> {
    ...
    match module_tag.as_ref() {
        ...
        str if str == "opencv feature detection".to_string() => Box::new(
            crate::modules::opencv_extractor::OpenCVExtractor::new(is_ini),
        ),
